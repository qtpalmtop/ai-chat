 ...
 *   - 主键：createdAt 升序
 *   - 次键：id 字典序（保证稳定 + 可预测）
 *     解释：nanoid 是随机串，字典序不具备业务含义，但"稳定"就够了——
 *     只要同一组输入永远得到同一组输出，下游的 React.memo / Vue shallowRef
 *     就能正确判断"数组是否变化"
 *
 * 不可变性：
 *   - 不修改入参数组
 *   - 不修改入参 message 对象（保持引用稳定，下游 useMemo / computed 能复用）
 *   - 返回新数组
 */
 ...
 * - 同 id 时优先保留 source 列表中的版本（更新状态的部分）
 */
